"""CRUD nad uslugama (bazni + podtip) i vezom radnik–usluga.

Usluga se u bazi dijeli na baznu tablicu `usluga` i jedan od pet podtipova
(NOKTI, DEPILACIJA_VOSKOM, …). Prema van je predstavljamo objedinjeno kroz
`UslugaOut` (kategorija + vrsta). Kreiranje/izmjena su transakcijski nad obje
tablice.
"""
...
def get_usluga_or_404(session: Session, usluga_id: int) -> USLUGA:
    """Bazni redak usluge (za internu upotrebu, npr. čitanje trajanja)."""
...
def _ucitaj_podtip(
    ...
    """Pronađi kojoj kategoriji usluga pripada i vrati (kategorija, vrsta)."""
...
def _u_out(session: Session, usluga: USLUGA) -> UslugaOut:
    """Spoji bazni redak s podtipom u izlazni oblik."""
...
def _veza_radnik_usluga(
    ...
    """Dohvat veze bez oslanjanja na redoslijed stupaca u složenom PK-u."""
    return session.exec(
...
def update_usluga(session: Session, usluga_id: int, data: UslugaUpdate) -> UslugaOut:
    """Ažuriraj cijenu/trajanje te opisni atribut postojećeg podtipa."""
...
def delete_usluga(session: Session, usluga_id: int) -> None:
    """Obriši uslugu; prvo veze i podtip (FK bez ON DELETE CASCADE), pa baznu."""
    usluga = get_usluga_or_404(session, usluga_id)
...
def list_radnici_za_uslugu(session: Session, usluga_id: int) -> list[RADNIK]:
    """Radnici koji nude zadanu uslugu (za odabir pri rezervaciji)."""
